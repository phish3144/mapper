...
export function describeError(error: unknown): string {
  ...
  if (code === '42501' || /row-level security/i.test(msg)) {
    return 'Dafür fehlen dir die Rechte. Möglicherweise bist du in diesem Arbeitsbereich nur Leser.'
  }
  if (code === '23505') {
    ...
    if (/workspace_invites_unique/.test(msg)) return 'Für diese Adresse liegt bereits eine Einladung vor.'
    if (/profiles_email_key/.test(msg)) return 'Für diese E-Mail-Adresse gibt es bereits ein Konto.'
    return 'Dieser Eintrag existiert bereits.'
  }
  if (code === '23514') return msg.replace(/^.*violates check constraint.*$/i, 'Die Eingabe ist ungültig.')
  ...
  if (/User already registered/i.test(msg)) return 'Für diese E-Mail-Adresse gibt es bereits ein Konto.'
